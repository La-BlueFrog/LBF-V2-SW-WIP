...
boolean_t    Acquire_Code_From_File( char* FileName, 
    ...
    Success = (UART_ReceiveData(UART1)==0x02);
    if (!Success)  // then stop here
    {
        return Success; 
    }
    ...
    UART_SendData(UART1, 0x01);  // SOH
    UART_SendData(UART1, (uint8_t) (ActualCodeSize &   0xFF) );       // LEN_LSB 
    UART_SendData(UART1, (uint8_t)((ActualCodeSize & 0xFF00)>>8)  );  // LEN_MSB

    // Get acknowledge from BTLE
    RecvdData = UART_ReceiveData(UART1);
    ...
    for (i=0; i<ActualCodeSize; i++)
    {
	UART_SendData( UART1, CodeBuffer[i] );
    }
    

    // Get CRC = XOR over code bytes from DA14580
    if (UART_ReceiveData(UART1)!= CodeCRC ) 
    {
        Success = FALSE;
        return Success;
    }
