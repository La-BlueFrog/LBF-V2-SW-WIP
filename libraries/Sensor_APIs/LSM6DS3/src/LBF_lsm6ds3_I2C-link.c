...
ReturnStatus_t 	LSM6DS3_IO_Init(void)
{
  ...
  if ( I2C2_ReadSingleReg(LSM6DS3_CHIPID, LSM6DS3_WHOAMI) == 0x69)
    return OK;
  else
    return NOK;
...
// and LSM6DS3_CHIPID  (different convention on conveying I2C Chip ID)
    ...
    I2C2_ReadMultipleReg (LSM6DS3_CHIPID, (uint16_t) RegisterAddr, pBuffer, NumByteToRead );
...
// and LSM6DS3_CHIPID  (different convention on conveying I2C Chip ID)
    ...
    I2C2_WriteMultipleReg (LSM6DS3_CHIPID, (uint16_t) RegisterAddr, pBuffer, NumByteToWrite );
