...
static boolean_t volatile ImpactDetected = FALSE;
...
{
OLED_PrintString("EXTI_9_5_IRQ_Handler\n");
  HAL_GPIO_EXTI_IRQHandler(INT2_ACC_GYR_PIN);
  // Checks if INT2_ACC_GYR_PIN is source of IT
  // If so: HAL clears the IT and calls HAL_GPIO_EXTI_Callback (see below)
  // with INT2_ACC_GYR_PIN as argument
}
...
void EXTI15_10_IRQHandler(void)
{
  ...
  HAL_GPIO_EXTI_IRQHandler(INT1_ACC_GYR_PIN);
  // Checks if INT1_ACC_GYR_PIN is source of IT
  // If so: HAL clears the IT and calls HAL_GPIO_EXTI_Callback (see below)
  // with INT1_ACC_GYR_PIN as argument

  HAL_GPIO_EXTI_IRQHandler(ONOFF_STAT_PIN);
  // Checks if ONOFF_STAT_PIN is source of IT
  // If so: HAL clears the IT and calls HAL_GPIO_EXTI_Callback (see below)
  // with ONOFF_STAT_PIN as argument
...
/**===========================================================================*/
/**==     Routines called by application when it has to wait for some IT      */
/**===========================================================================*/

void Wait_For_Impact(void)
{
   ImpactDetected = FALSE;  // reset flag
   while (!ImpactDetected) ;  // wait until flag is set by IT Service Routine

   // Switch to "FIFO" Mode (FIFO_CTRL5) to store data from now on,
   // stop when FIFO full
   I2C2_RmodWSingleReg ( LSM6DS3_CHIPID, 
			LSM6DS3_XG_FIFO_CTRL5,
			LSM6DS3_XG_FIFO_MODE_MASK, 
			LSM6DS3_XG_FIFO_MODE_FIFO );
